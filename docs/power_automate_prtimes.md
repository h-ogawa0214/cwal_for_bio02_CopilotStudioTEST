...
- `condition` に `search_word` / `year` / `skip` があり、API 側で絞り込める可能性がある。
  ただし 1 語しか渡せないと思われ、6 語の OR には 6 リクエスト必要。総件数が少ない
  （この社は 5 件）ため、全件取得＋ローカルフィルタのほうが効率がよい。**使わない**
...
**`releaseTypeName` について。** この記事は `"イベント"` で、`config/criteria.md` の
discard 条件「イベント・セミナー・説明会の告知」に該当する。有力な事前フィルタになりうるが、
**取り得る値の全体を把握していないため、当面はハード除外に使わない。** `candidates` に記録し、
判定プロンプトへの入力シグナルとして渡す。数回の実行で出現値を集めてから判断する。
...
疎通確認の段階では `lookback_days` を 400 程度にして、まず行が入ることを確認する。
...
3. **`limit` パラメータの上限** — 90 を超える指定や、上限超過時の API の挙動は未確認。
   実測した 1 社は `total: 5` だったが、18 社ぶんの `total` は未確認。ページング
   （`skip`）が必要な社があるかは全社を 1 度叩けば判明する
4. **`releaseTypeName` の取り得る値** — `"イベント"` 以外を未確認。事前フィルタに使うなら
   出現値を集める必要がある
5. **`__NEXT_DATA__` 構造の安定性** — PR TIMES 側の実装詳細であり、予告なく変わりうる。
   A-6 ② の条件で検知できるようにしてある
